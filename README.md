...
> **版本 0.2.0** — 用 pygame 复刻《交响乐之雨》(Symphonic Rain) 的演奏模式
...
1. 到 **Release** 页面下载 `SymphonicRain-Windows-0.2.0.zip`
...
| **Tab** | AUTO 自动演奏（右上角显示，期间不记录成绩） |
| **空格** | 暂停 / 继续（暂停时快照得分·连击·亮灯，界面冻结；继续 = 从暂停秒数重新进入并恢复成绩） |
| **← / →** | 前后跳转（默认 7 秒，得分·连击·亮灯保留） |
| ESC | 返回菜单 |
...
- 例如朵鲁妲主题曲《秘密》(SR03 / SR13)：**50% 起始**附近正是密集和弦连打的难点段落，适合专项起手练习
- 高难度挑战推荐：法珞主题曲 **《雨之音》**（雨のmusique，SR05 / SR14）

### ⏪ ← 键回跳 — 段落反复练习
演奏中按 **←** 可回退（默认 **7 秒**）重新演奏刚才的段落，得分、连击与亮灯全部保留，**循环往复直到打顺为止**；配合 **→** 前进可精确控制练习区间。手滑错过一串音符？按一下 ← 立刻重来，不用等整曲结束。

> 想从头重练一小节时：← 回退 7 秒 → 反复打到全 Perfect → 再 ← 继续下一段，等于把整首曲子拆成小节逐个攻破。
...
演奏中 **← / →** 的前后跳转时长在 `config.py` 中定义：

```python
SEEK_SECONDS = 7.0    # 演奏中 ←/→ 前后跳转时长(秒)
```

- 改大（如 `10.0`）跳得更远，适合跨段落移动；改小（如 `3.0`）更精细，适合逐句打磨
- 修改后重启游戏生效
